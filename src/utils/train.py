        ...
        for input_batch, target_batch in train_loader:
            ...
            if global_step % evaluation_frequency == 0:
                train_loss, validation_loss = evaluate_model(
                    model, train_loader, validation_loader, device, evaluation_steps
                )
                ...
                print(
                    f"Epoch [{epoch+1}/{num_epochs}], Step [{global_step:03d}], "
...
def evaluate_model(model, train_loader, validation_loader, device, evaluation_steps):
    ...
    with torch.no_grad():
        ...
        validation_loss = loader_loss(
            validation_loader, model, device, evaluation_steps
        )
